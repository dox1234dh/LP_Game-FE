<template>
  <div class="h-auto content-center space-y-6 py-10">
    ...
    <div class="flex justify-center items-center">
      <div
        class="grid grid-cols-1 xs:grid-cols-2 md:grid-cols-4 3xl:grid-cols-5 gap-10"
      >
        ...
        >
          ...
          <template #actions>
            ...
            <div
              v-if="item.id"
              class="flex space-x-1 justify-center items-center h-8"
            >
              <span>Trạng thái: </span>
              <a-tag :color="arrayStatus[item.status]?.color">{{
                arrayStatus[item.status]?.title
              }}</a-tag>
            </div>
            <a-button v-if="item.id" @click="playGame(item.id)"
              >Tìm hiểu</a-button
            >
            <a-button v-else @click="detailBlock(item.key)"
